    ...
    <div style={{ paddingTop: '80px', minHeight: '100vh', backgroundColor: 'var(--color-surface-secondary)' }}>
      ...
      <div className="container" style={{ padding: '2rem 1.5rem', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          {/* Social Sharing */}
          <div className="card" style={{ padding: '2rem' }}>
            <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>Share this property</h2>
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <a href={`https://wa.me/?text=${encodeURIComponent(`Check out this amazing property on SwiftSpaces: ${property.title} for ${property.price}. View here: https://swiftspace.africa/properties/${property.id}`)}`} target="_blank" rel="noopener noreferrer" className="btn" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#25D366', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '8px', textDecoration: 'none', fontWeight: 600 }}>
              ...
              <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://swiftspace.africa/properties/${property.id}`)}`} target="_blank" rel="noopener noreferrer" className="btn" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#1877F2', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '8px', textDecoration: 'none', fontWeight: 600 }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                Share on Facebook
              </a>
              <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out this amazing property on SwiftSpaces: ${property.title}`)}&url=${encodeURIComponent(`https://swiftspace.africa/properties/${property.id}`)}`} target="_blank" rel="noopener noreferrer" className="btn" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#000000', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '8px', textDecoration: 'none', fontWeight: 600 }}>
                ...
                <>
                  ...
                  {property.agent?.phoneNumber && (
                    <div style={{ marginTop: '1.5rem', display: 'flex', gap: '0.75rem' }}>
                      <a href={`tel:${property.agent.phoneNumber}`} className="btn btn-outline" style={{ padding: '0.75rem', flex: 1, fontSize: '0.875rem', display: 'flex', justifyContent: 'center', gap: '0.5rem' }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
                        Call
                      </a>
                      <a href={`https://wa.me/${property.agent.phoneNumber.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hi, I'm interested in your property on SwiftSpaces: ${property.title}`)}`} target="_blank" rel="noopener noreferrer" className="btn btn-outline" style={{ padding: '0.75rem', flex: 1, fontSize: '0.875rem', display: 'flex', justifyContent: 'center', gap: '0.5rem', borderColor: '#25D366', color: '#25D366', backgroundColor: 'rgba(37, 211, 102, 0.05)' }}>
